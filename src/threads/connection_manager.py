...
def connection_manager(robot):
    ...
    while True:
        ...
        if (delta > timeout):
            ...
            if g.debug_connection_manager: print(f'Robot with ip {robot.IP} timed out')
            return

        # Terminate if Robot no longer exists
        with g.visible_mutex and g.lost_mutex:
            if ((robot not in g.visible) and (robot not in g.lost)):
                if g.debug_connection_manager: print(f'Exiting Connection Manager. Robot with ip {robot.IP} is Not in Visible or Lost List')
                return