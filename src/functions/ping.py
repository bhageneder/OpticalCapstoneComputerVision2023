...
def multi_ping(dest_addr, count, interval, timeout):
    ...
    for sequence in range(count):
        for transceiver_number in range(8):
            ...
            payload = f'{global_vars.ROBOT_IP_ADDRESS}_T_{transceiver_number}'
    ...
    if global_vars.debug_maintenance: print(f'{thread_name}: Lowest Ping Loss Percentage: {lowest_loss_percentage}')
