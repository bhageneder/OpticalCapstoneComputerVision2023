...
def multi_ping(dest_addr, count, interval, timeout):
    ...
    try:
        ...
        if e.errno == errno.EPROTONOSUPPORT:
            # Sometimes, the OS fails to open a new ICMP socket...
            if g.debug_maintenance is True: print(f'{thread_name}: ICMP Socket Failed to Open')
        else:
            if g.debug_maintenance is True: print(f'{thread_name}: Unknown Socket Error')
        return -1
    
    ICMP_socket.settimeout(timeout)
    dest_ip = socket.gethostbyname(dest_addr)
    if g.debug_maintenance is True: print(f'{thread_name}: PINGING {dest_addr}')
    ...
    for sequence in range(count):
        for transceiver_number in range(8):
            ...
            # more information why. (Strange behavior was happening due to light reflection or something else...)
            # The payload is used here for extra verification later on.
            payload = f'{g.ROBOT_IP_ADDRESS}_T_{transceiver_number}'
        ...
        while True:
            try:
                ...
                if received_transceiver_number >= 0 and received_transceiver_number <= 7 and received_maintenance_thread_number == maintenance_thread_number and received_sequence == sequence and response_payload == f'{g.ROBOT_IP_ADDRESS}_T_{received_transceiver_number}':
                    num_received_pings_per_transceiver[received_transceiver_number] += 1
                    total_rtt_time_per_transceiver[received_transceiver_number] += elapsed_time
                    if g.debug_maintenance is True: print(f'{thread_name}: Ping_{received_transceiver_number}, {len(response)} bytes from {addr[0]}: icmp_seq={received_sequence} ttl={response[8]} time={elapsed_time:.2f} ms')

            except socket.timeout:
                # When all pings have been sent, and no more data is received, any ping requests that did not
                # receive replies are considered timed out.
                if g.debug_maintenance is True: print(f'{thread_name}: All Unresponsive Ping Requests Have Timed Out')
                break

    if g.debug_maintenance is True: print(f'{thread_name}: Received Pings Array {num_received_pings_per_transceiver}')
    ...
    if g.debug_maintenance: print(f'{thread_name}: Lowest Ping Loss Percentage: {lowest_loss_percentage}')
    ...
    if g.debug_maintenance is True: print(f'{thread_name}: Best Transceiver {best_transceiver_number}, --- {dest_addr} ping statistics ---')
    if g.debug_maintenance is True: print(f'{thread_name}: Best Transceiver {best_transceiver_number}, {count} packets transmitted, {highest_received_packets} received, {lowest_loss_percentage:.2f}% packet loss, time {(count * interval * 1000)}ms')
    if g.debug_maintenance is True: print(f'{thread_name}: Best Transceiver {best_transceiver_number}, RTT avg = {avg_rtt:.2f} ms')
...
def ping(dest_addr, identifier, count, interval, timeout, payload):
    ...
    try:
        ...
        if e.errno == errno.EPROTONOSUPPORT:
            # Sometimes, the OS fails to open a new ICMP socket...
            if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: ICMP Socket Failed to Open')
        else:
            if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: Unknown Socket Error')
        loss_percentage = 100
        return loss_percentage
    
    ICMP_socket.settimeout(timeout)
    dest_ip = socket.gethostbyname(dest_addr)
    if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: PING {dest_addr} ({dest_ip}): {len(payload)} data bytes')
        ...
        while True:
            try:
                ...
                if (received_identifier == identifier and 
                    ...
                    received_packets += 1
                    if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: {len(response)} bytes from {addr[0]}: icmp_seq={sequence} ttl={response[8]} time={elapsed_time:.2f} ms')
                    total_rtt += elapsed_time
                    break

            except socket.timeout:
                if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: Ping Request Timed Out')
                break
    ...
    if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: --- {dest_addr} ping statistics ---')
    if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: {count} packets transmitted, {received_packets} received, {loss_percentage:.2f}% packet loss, time {(count * interval * 1000)}ms')
    if g.debug_mini_maintenance is True: print(f'Ping_{identifier}: RTT avg = {avg_rtt:.2f} ms')
